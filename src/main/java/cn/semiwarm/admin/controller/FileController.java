...
public class FileController {
    ...
    @ResponseBody
    public UploadImageResponse uploadImages(HttpSession session, HttpServletRequest request, HttpServletResponse response, @PathVariable("type") String type, @RequestParam(value = "images") MultipartFile[] uploadImages) throws Exception {

        UploadImageResponse imageResponse = new UploadImageResponse(); // 初始化响应结果

        try {

            if (null != uploadImages) {
                // 初始化上传结果
                Image image = initUploadImage(session, request, response, uploadImages[0], type);
                // 创建待上传的文件
                File uploadFile = new File(image.getImageRealPath());
                FileUtils.copyInputStreamToFile(uploadImages[0].getInputStream(), uploadFile);
